...
class GetProxy(object):
    ...
    def processPool(self):
        ...
        for i in range(2, 5):
            ...
            # 调用获取ip函数
            ip_list = self.get_free_url(url)

            # 创建线程池
            pool = Pool(processes=5)
            for ip in ip_list:
                # 进程池启动将所有ip存入进程池
                res = pool.apply_async(func=self.test_ip, args=(ip,))
                # 将所有ip存入列表
    ...
    # 解析网站部分,不通用需要单写
    # 执行到这个函数报错,相当于占位符
    def get_free_url(self, url):
        assert (0)


# 继承父类
class ParseXici(GetProxy):
